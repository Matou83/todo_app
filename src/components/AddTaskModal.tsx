...
interface Props {
  ...
  categories: Category[]
  task?: Task
  onSave: (title: string, description: string, status: Status, priority: Priority, categoryId: string, editId?: string, dueDate?: number, subtasks?: Subtask[]) => Promise<boolean>
...
export default function TaskModal({ defaultStatus, columns, categories, task, onSave, onAddCategory, onUpdateSubtasks, onClose }: Props) {
  const isEdit = !!task
  ...
  function applySubtasks(next: Subtask[]) {
    setSubtasks(next)
    if (isEdit && task && onUpdateSubtasks) onUpdateSubtasks(task.id, next)
  }

  function toggleSubtask(id: string) {
    applySubtasks(subtasks.map(s => s.id === id ? { ...s, done: !s.done } : s))
  }

  function addSubtask() {
    const trimmed = newSubtaskTitle.trim()
    if (!trimmed) return
    applySubtasks([...subtasks, { id: crypto.randomUUID(), title: trimmed, done: false }])
    setNewSubtaskTitle('')
    subtaskInputRef.current?.focus()
  }

  function deleteSubtask(id: string) {
    applySubtasks(subtasks.filter(s => s.id !== id))
  }
  ...
  async function handleClose() {
    ...
    if (isEdit && hasChanges()) {
      ...
      const ok = await onSave(title.trim(), description.trim(), status, priority, categoryId, task?.id, dueDateTs, subtasks)
      setSaving(false)
          ...
          <div>
              ...
              <div className="h-1.5 bg-slate-100 rounded-full mb-3 overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: subtasks.every(s => s.done) ? '#16a34a' : '#14b8a6',
                  }}
                />
              </div>
            )}
            ...
            {subtasks.length > 0 && (
              <div className="flex flex-col gap-1.5 mb-3">
                {subtasks.map(s => (
                  <div key={s.id} className="flex items-center gap-2.5 group/subtask">
                    <button
                      type="button"
                      ...
                      className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors duration-150 cursor-pointer ${
                        s.done ? 'bg-teal-500 border-teal-500' : 'border-slate-300 hover:border-teal-400'
                      }`}
                    ...
                    <span className={`flex-1 text-sm transition-colors duration-150 ${s.done ? 'line-through text-slate-400' : 'text-slate-700'}`}>
                      {s.title}
                    </span>
                    <button
                      type="button"
                      onClick={() => deleteSubtask(s.id)}
                      className="opacity-0 group-hover/subtask:opacity-100 transition-opacity duration-150 cursor-pointer text-slate-300 hover:text-red-400"
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 text-sm border border-slate-200 rounded-xl px-3 py-2 text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent transition-shadow"
              />
              <button
                type="button"
                onClick={addSubtask}
                disabled={!newSubtaskTitle.trim()}
                className="px-3 py-2 text-sm font-semibold text-teal-600 hover:text-teal-800 disabled:text-slate-300 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
                Ajouter
              </button>
            </div>
